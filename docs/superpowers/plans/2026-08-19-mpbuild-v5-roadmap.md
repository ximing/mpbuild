...
> 当前开工：P0 → `docs/superpowers/plans/2026-08-19-mpbuild-v5-p0-graph-kernel.md`

## 阶段

| 阶段 | 计划文件 | 可独立验收 |
|---|---|---|
| P0 图内核 | `2026-08-19-mpbuild-v5-p0-graph-kernel.md` | `mpb5 inspect graph` 打出节点/边；假 adapter 快照通过 |
| P1 可构建 | 待 P0 完成后另写 | `mpb5 build` 打出页面四件套；`plugin://` 不失败 |
...
1. 工作在分支 `feat/v5-p0-graph-kernel` 的隔离 worktree，不直接改用户当前 checkout 的 master 工作区。
