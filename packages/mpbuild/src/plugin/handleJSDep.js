...
const fs = require('fs');
const { transformSync, Visitor } = require('@swc/core');
                ...
                if (/\.(js|wxs)$/.test(asset.outputFilePath) && asset.contents) {
                    ...
                    class ImportVisitor extends Visitor {
                        visitCallExpression(e) {
                            if (e.callee.type === 'CallExpression') {
                                
                            }
                        }

                        visitExpression(astPath) {
                        ...
                        visitImportDeclaration(astPath) {
                            ...
                            const libPath = res.resolveLib;
                            let { outputPath: libOutputPath } = mpb.hooks.rewriteOutputPath.call({
                                filePath: libPath,
                                asset,
                                depType: 'js',
                            });
                    ...
                    const out = transformSync(code, {
                        plugin: (m) => new ImportVisitor().visitProgram(m),
                    });
                    asset.contents = out.code;
                }
            } catch (e) {
